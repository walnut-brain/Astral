...
namespace AstralTests.Payloads
{
    public class SerializationTests
    {
        ...
        public void JsonSerializationTestUtf8()
        {
            ...
            var pair = ser(new ContentType("text/json; charset=" + Encoding.UTF8.WebName)).Head(test).Unwrap();
            Assert.Equal<string>("text/json; charset=utf-8", pair.Item1.ToString());
            var d = (ChieldTestContract) deser(pair.Item1).Head(typeof(ChieldTestContract), pair.Item2).Unwrap();
        ...
        public void JsonSerializationTest()
        {
            ...
            var pair = ser(new ContentType("text/json")).Head(test).Unwrap();
